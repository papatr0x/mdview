...
**Hiding the markers is deliberately *not* a second exception.** The `**`, `*` and `_` around emphasis are not drawn, nor the backticks around inline code, the `#` opening a heading or the `>` opening each line of a quote, but they are still in the string: the renderer marks them with an `.hiddenMarkdownDelimiter` attribute and `AppearanceAwareTextView`, as the layout manager's delegate, gives those characters a `.null` glyph — no drawing, no width, character untouched. Deleting them would have been less code and was rejected for exactly this reason: copying a bold phrase out of the view still yields the `**` the file has, and every range the plan recorded still lines up with the source. Reach for the same trick before reaching for `replaceCharacters`.

One consequence to know about: a null glyph has no width, so TextKit packs it onto the end of the *previous* line's fragment. Anything drawn per line for a range that starts on a hidden marker — the blockquote bars — must anchor on drawn content instead of on the range, or it lands a line too high (`AppearanceAwareTextView.drawnGlyph(on:in:_:)`).
...
**Styling is centralized in `MarkdownStyle`** (`Sources/mdview/Markdown/MarkdownStyle.swift`), which resolves a `ColorTheme` + font preference + light/dark appearance into concrete `NSFont`/`NSColor` per `MarkdownNodeKind` (`Sources/mdview/Markdown/ColorTheme.swift`). There are two user-chosen fonts, each with its own family and size: a body font for everything that is not code, and a code font for inline code and fenced blocks, which the settings restrict to fixed-width families (the body font has no such restriction, so a proportional body font is allowed and does break the column alignment of markers and indentation). Beyond that split, color and — for a few kinds — weight/slant differ, and headings differ by *size*: `headingFont(level:)` scales the body size per level, strictly decreasing and never landing on the body size. Headings carry no color of their own, which is why `MarkdownNodeKind` has no heading cases. An unavailable code family falls back to the system monospaced font, which is what code used unconditionally before the family became configurable. Two things set `NSParagraphStyle` — the list-item spacing, on the item's *first line* only, and a blockquote's indent, over the whole quote — and they meet whenever a list sits inside a quote. Because `addAttribute(.paragraphStyle:)` replaces the whole style rather than merging into it, both go through `MarkdownRenderer.mutateParagraphStyle(of:in:_:)`, which copies whatever is already on the range and changes that. Any third paragraph-level attribute must do the same. `ColorTheme` holds two full `ColorPalette`s (light/dark), each mapping every `MarkdownNodeKind` to a color plus a dedicated code-block background color, and is `Codable`/persisted as JSON in `UserDefaults`.
